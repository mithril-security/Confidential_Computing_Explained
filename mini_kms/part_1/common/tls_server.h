...
namespace Https {
    class TlsConnection {
        ...
        public: 
            ...
            mbedtls_pk_context get_pk(){ return pkey; }
                //net file descriptors
            mbedtls_net_context get_listen_fd() { return listen_fd; }
            mbedtls_net_context get_client_fd() { return client_fd; }
            // setters
            void set_x509_cert(mbedtls_x509_crt cert) { srvcert = cert; }
            void set_private_key(mbedtls_pk_context pk) { pkey = pk; }

            // ssl configuration initialization & modules
            int init_server();
            // starts the server and handles the connections
            int start_server(const char* port); 

            // load of private key and certificate associated
